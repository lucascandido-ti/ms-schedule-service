...
export class EntityAlreadyExistsException<
  T extends Entity<TId>,
  TId = T['id'],
> extends Exception {
  constructor(entity: T) {
    super(
      `${entity.constructor.name} entity with ID ${entity.id} already exists`,
    );
  }
}
...
export function ReadRepositoryEx<
  ...
  TId = T['id'],
  TBase extends Constructor<IReadRepository<T, TId>> = Constructor<
...
export function RepositoryEx<
  T extends AggregateRoot<TId>,
  TId = T['id'],
  TBase extends Constructor<IRepository<T, TId>> = Constructor<
    IRepository<T, TId>
  >,
>(Base: TBase) {
 ...
 */
export interface IRepositoryWithEvents<
  T extends AggregateRoot<TId>,
  TId = T['id'],
